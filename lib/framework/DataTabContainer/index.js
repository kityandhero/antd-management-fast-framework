    ...
    _this.doWorkWhenDidUpdate = function(preProps, preState, snapshot) {
      ...
      if (!dataLoading) {
        if (
          ...
          _this.redirectToPath(
            ''.concat(pathname.replace('/update/', '/load/')),
          );
        }
      }
    };
    ...
    _this.handleTabChange = function(key) {
      ...
      (_this.tabList || []).forEach(function(item) {
        if (item.key === key) {
          _this.redirectToPath(
            ''
              .concat(match.url.replace('/update', '/load'), '/')
              .concat(item.key),
          );
        }
      });
        ...
        value: function render() {
          ...
          if (customTabActiveKey) {
            return /*#__PURE__*/ _react['default'].createElement(
              _proLayout.PageHeaderWrapper,
              {
                ...
                content: (0, _FunctionComponent.pageHeaderContent)(
                  pageHeaderContentDataConfig,
                ),
                extraContent: (0, _FunctionComponent.pageHeaderExtraContent)(
                  this.pageHeaderExtraContent(),
                ),
          ...
          return /*#__PURE__*/ _react['default'].createElement(
            _proLayout.PageHeaderWrapper,
            {
              ...
              extraContent: (0, _FunctionComponent.pageHeaderExtraContent)(
                this.pageHeaderExtraContentData(),
              ),
              tabList: tabListAvailable, // tabBarExtraContent={<Button>Extra Action</Button>}
              onTabChange: this.handleTabChange,
              tabProps: this.buildOtherTabProps(), // onBack={() => {
    ...
    [
      {
        ...
        value: function getDerivedStateFromProps(nextProps, prevState) {
          return _get(
            _getPrototypeOf(DataTabContainer),
