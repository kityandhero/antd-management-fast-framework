...
function _storageAssist() {
  const data = require("@/customConfig/storageAssist");

  _storageAssist = function _storageAssist() {
    return data;
  };
...
request.interceptors.request.use( /*#__PURE__*/function () {
  var _ref = _asyncToGenerator(function* (url, options) {
    const token = (0, _storageAssist().getToken)() || 'anonymous';
    const corsUrl = (0, _tools.corsTarget)(); // const url = transferToVirtualAccess() ? urlParam : `${corsUrl}${urlParam}`;
    ...
    if (token) {
      ...
      headers[`${(0, _storageAssist().getTokenKeyName)()}`] = token;
      return {
        url: urlChange,
