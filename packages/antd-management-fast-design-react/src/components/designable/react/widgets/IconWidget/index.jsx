...
export const IconWidget = observer((properties) => {
  ...
  const size = properties.size >= 0 ? properties.size : '1em';
...
const ShadowSVG = (properties) => {
  ...
  useEffect(() => {
    if (reference.current) {
      const root = reference.current.attachShadow({
        mode: 'open',
      });

      root.innerHTML = `<svg viewBox="0 0 1024 1024" style="width:${width};height:${height}">${properties.content}</svg>`;
    }
