...
import { MonacoInput } from 'antd-management-fast-design-react';

const printAttribute = (node) => {
  if (!node) {
    return '';
  }
  ...
  if (node.depth !== 0) {
    properties.name = node.props?.name || node.id;
  }
...
const transformToMarkupSchemaCode = (tree) => {
...
export const MarkupSchemaWidget = (properties) => {
  return (
    <MonacoInput
      {...properties}
      options={{ readOnly: true }}
      value={transformToMarkupSchemaCode(properties.tree)}
      language="typescript"
    />
  );
};
